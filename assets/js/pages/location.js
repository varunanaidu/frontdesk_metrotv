$(function() {
    ...
    $('#btn-addnewloc').on('click', function() {
    ...
    $("#location-form").ajaxForm({
        ...
        beforeSubmit: function() {
            $('#submitLoc').removeClass('btn-primary').addClass('btn-warning').prop('disabled', true);
        },
        ...
        complete: function() {
            $('#submitLoc').removeClass('btn-warning').addClass('btn-primary').prop('disabled', false);
        }
    });
    ...
    t.on('click', '#btn-editloc', function(event) {
        event.preventDefault();
        var row_id = $(this).data('id');
        $.ajax({
            url: base_url + "location/find",
            type: 'post',
            data: {
                'key': row_id
            },
            ...
            success: function(data) {
                if (data.type != 'done') {
                    ...
                    $('#location_name').val(data.msg[0].name);
    ...
    t.on('click', '#btn-deleteloc', function() {
        var row_id = $(this).data('id');
        Swal.fire({
            title: 'Hapus data !',
            type: 'warning',
            html: '<span class="italic">Apakah anda yakin ingin menghapus data location ini ?</span>',
