...
class Userfd extends MY_Controller {
	...
	public function index()
	{
		...
		$this->fragment['js'] = [ 
			...
			base_url('assets/js/pages/user.js') 
		];

		$this->fragment['role'] = $this->sitemodel->view('userRole', '*');
		$this->fragment['pagename'] = 'pages/user_page.php';
	...
	public function view()
	{
		...
		foreach ($res as $row) {
			...
			if ( $this->log_role == 1 ) {
				$button .= '
					<button id="btn-edituser" class="btn btn-success btn-circle btn-sm" data-id="'.$row->id.'"><i class="fas fa-pencil-alt"></i></button> 
					<button id="btn-deleteuser" class="btn btn-danger btn-circle btn-sm" data-id="'.$row->id.'"><i class="fas fa-trash"></i></button>';
			}
	...
	public function save()
	{
		...
		if ($type == 'update') {
			$this->sitemodel->update('user', $data, ['id'=>$id]);
		}else{
			$id = $this->sitemodel->insert('user', $data);
		}
	...
	public function delete(){
		...
		if (!$check) {$this->response['msg'] = "No data found.";echo json_encode($this->response);exit;}
		// Delete 
		$this->sitemodel->delete('user', ['id'=>$key]);
		/*** Result Area ***/
		$this->response['type'] = 'done';
